...
const restServer = setupServer(
  ...
  rest.post(loginURL, async (req, res, ctx) => {
    ...
    if (user.email === "user1111@gmail.com") {
      return res(ctx.json(
        {id: 'userID', name: 'Test User 1111', roles: ['member', 'moderator']},
      )) 
    }
    ...
    else if (user.email === "user4444@gmail.com") {
      return res(ctx.json(
        {id: 'userID2', name: 'Test User 4444', roles: ['member']},
      ))
    }
...
test('OK', async () => {
  ...
  await request.post('/api/graphql')
    ...
    .then((res) => {
      console.log('resresresres ', res.body)
      expect(res).toBeDefined();
      expect(res.body).toBeDefined();
      expect(res.body.data.login.name).toEqual('Test User 1111');
      expect(res.body.data.login.accessToken).toBeDefined();
    });
});

test('Login not moderator', async () => {
...
test('Moderator', async () => {
  const nobbyToken = await login.asTest1(request);
  await request.post('/api/graphql')
    .set('Authorization', 'Bearer ' + nobbyToken)
    .send({query: `{getFlaggedProducts {
      id, seller, name, price, description
    }}`})
    .expect(200)
    .expect('Content-Type', /json/)
    .then((res) => {
...
test('No Bearer', async () => {
  accessToken = await login.login(request, login.test1);
  await request.post('/api/graphql')
